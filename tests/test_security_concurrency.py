...
def _next_open_slot(business_id=BUSINESS_ID):
    ...
    timezone = (
        settings["timezone"] if settings and settings["timezone"] else appointments.DEFAULT_TIMEZONE
    )
    try:
...
class AtomicSecurityTests(unittest.TestCase):
    ...
    def _invitation(self):
        ...
        return (
            result["business_id"],
            approved["invitation_url"].rsplit("/", 1)[-1],
            result["user_id"],
        )
    ...
    def test_same_invitation_only_one_concurrent_acceptance(self):
        ...
        threads = [
            threading.Thread(target=worker, args=(f"Password-{i}-secure",)) for i in range(2)
        ]
    ...
    def test_reinvite_revokes_old_and_preserves_history(self):
        ...
        self.assertEqual(
            platform.accept_invitation(business_id, token_a, "Password-A-secure")["reason"],
            "invalid_token",
        )
        self.assertTrue(
            platform.accept_invitation(business_id, token_b, "Password-B-secure")["success"]
        )
        self.assertEqual(
            platform.accept_invitation(business_id, token_b, "Password-C-secure")["reason"],
            "invalid_token",
        )
        connection = database.get_connection()
        rows = connection.execute(
            "SELECT revoked_at, used_at FROM invitations WHERE business_id = ? ORDER BY id",
            (business_id,),
        ).fetchall()
        connection.close()
    ...
    def test_email_claim_allows_one_worker_and_retry_after_failure(self):
        ...
        appointment = appointments.create_appointment(
            "Ana", "3815000000", "Corte", date, time, BUSINESS_ID, email="ana@example.com"
        )
        self.assertTrue(
            appointment["success"],
        ...
        results = []
        with (
            mock.patch.dict("os.environ", {"SMTP_HOST": "smtp.test"}),
            mock.patch.object(notifications, "_send_email", side_effect=fake_send),
        ):
            threads = [
                threading.Thread(
                    target=lambda: results.append(
                        notifications.send_confirmation_email(BUSINESS_ID, apt, force=True)
                    )
                )
                for _ in range(2)
            ]
        ...
        self.assertEqual(
            database.get_notification_state_scoped(
                BUSINESS_ID, apt["id"], notifications.CONFIRMATION, "email"
            )["status"],
            "sent",
        )
