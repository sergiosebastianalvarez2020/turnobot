...
class TestBusinessMigration(unittest.TestCase):
    ...
    def snapshot(self, connection):
        ...
        for table in ("business_settings", "services", "weekly_schedules", "appointments"):
            result[table] = [
                dict(row) for row in connection.execute(f"SELECT * FROM {table} ORDER BY id")
            ]
        return result
    ...
    def test_migrates_new_database(self):
        ...
        self.assertEqual(
            connection.execute("SELECT COUNT(*) FROM weekly_schedules").fetchone()[0], 7
        )
        self.assertEqual(connection.execute("SELECT COUNT(*) FROM appointments").fetchone()[0], 0)
        self.assertEqual(
            connection.execute("SELECT COUNT(*) FROM services WHERE business_id = 1").fetchone()[0],
            3,
        )
        self.assertEqual(
            connection.execute(
                "SELECT COUNT(*) FROM weekly_schedules WHERE business_id = 1"
            ).fetchone()[0],
            7,
        )
    ...
    def test_migrates_existing_database_without_changing_data(self):
        ...
        shutil.copy(
            ROOT / "migrations" / "003_businesses.sql", self.migrations_dir / "003_businesses.sql"
        )
        database.init_database()
    ...
    def test_new_constraints_and_indexes(self):
        ...
        connection = self.connect()
        connection.execute(
            "INSERT INTO businesses (name, slug) VALUES (?, ?)", ("Business B", "business-b")
        )
        business_b = connection.execute(
            "SELECT id FROM businesses WHERE slug = 'business-b'"
        ).fetchone()[0]
        connection.execute(
            "INSERT INTO weekly_schedules (day_of_week, business_id) VALUES (?, ?)", (0, business_b)
        )
        with self.assertRaises(sqlite3.IntegrityError):
            connection.execute(
                "INSERT INTO weekly_schedules (day_of_week, business_id) VALUES (?, ?)",
                (0, business_b),
            )
        connection.execute(
            "INSERT INTO appointments (customer_name, service, appointment_date, appointment_time, business_id) VALUES (?, ?, ?, ?, ?)",
            ("A", "Corte", "2099-01-02", "09:00", 1),
        )
        connection.execute(
            "INSERT INTO appointments (customer_name, service, appointment_date, appointment_time, business_id) VALUES (?, ?, ?, ?, ?)",
            ("B", "Corte", "2099-01-02", "09:00", business_b),
        )
        with self.assertRaises(sqlite3.IntegrityError):
            connection.execute(
                "INSERT INTO appointments (customer_name, service, appointment_date, appointment_time, business_id) VALUES (?, ?, ?, ?, ?)",
                ("A2", "Corte", "2099-01-02", "09:00", 1),
            )
        connection.execute(
            "INSERT INTO appointments (customer_name, service, appointment_date, appointment_time, status, business_id) VALUES (?, ?, ?, ?, 'cancelled', ?)",
            ("A3", "Corte", "2099-01-02", "09:00", 1),
        )
