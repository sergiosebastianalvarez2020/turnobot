...
class RewardsAndRetentionTests(LoyaltyBase):
    def setUp(self):
        ...
        if not account:
            self._execute(
                "INSERT INTO loyalty_accounts (business_id, customer_phone, points_balance) VALUES (1,'3815000001',100)"
            )
            self._execute(
                "INSERT INTO points_ledger (business_id, account_id, delta, type, reason) VALUES (1, (SELECT id FROM loyalty_accounts WHERE business_id=1 AND customer_phone='3815000001'), 100, 'adjust', 'test')"
            )
    ...
    def test_reward_redeem_is_idempotent_and_ledger_linked(self):
        ...
        try:
            row = connection.execute(
                "SELECT COUNT(*) c FROM redemptions WHERE business_id=1 AND idempotency_key='same-key'"
            ).fetchone()
        finally:
            connection.close()
    ...
    def test_retention_uses_normalized_phone_and_2_60_rule(self):
        ...
        for day in (old, (date.today() - timedelta(days=90)).isoformat()):
            self._execute(
                "INSERT INTO appointments (customer_name, phone, service, appointment_date, appointment_time, appointment_end, duration, status, business_id) VALUES (?,?,?,?,?,? ,60,'completed',1)",
                ("Cliente", "+54 381-500-0001", "Corte", day, "10:00", "11:00"),
            )
    ...
    def test_concurrent_redeems_cannot_overspend(self):
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            results = list(
                pool.map(
                    lambda key: loyalty.redeem(1, self.account["id"], reward["id"], key), ("a", "b")
                )
            )
        self.assertEqual(sum(result["success"] for result in results), 1)
        self.assertEqual(loyalty.get_balance(1, "3815000001"), 20)
        self.assertEqual(
            len([row for row in self._ledger(1, self.account["id"]) if row["type"] == "redeem"]), 1
        )
    ...
    def test_admin_http_csrf_and_tenant_scope(self):
        ...
        response = self.client.post(
            ...
            data={"name": "HTTP", "points_cost": "10", "csrf_token": token},
        )
        self.assertEqual(response.status_code, 302)
        bad_csrf = self.client.post(
            "/admin/fidelizacion/recompensas", data={"name": "No", "points_cost": "10"}
        )
        self.assertEqual(bad_csrf.status_code, 400)
        reward = loyalty.list_rewards(1)[0]
        denied = self.client.post(
            f"/b/business-b/admin/fidelizacion/recompensas/{reward['id']}/estado",
    ...
    def test_http_recuperacion_on_off_y_csrf(self):
        ...
        for index, status in enumerate(("completed", "completed", "cancelled", "no_show")):
            self._execute(
                "INSERT INTO appointments (customer_name, phone, service, appointment_date, appointment_time, appointment_end, duration, status, business_id) VALUES (?,?,?,?,?,? ,60,?,1)",
                (
                    "Recuperable",
                    "3815000099",
                    "Corte",
        ...
        bad = self.client.post(
            "/admin/fidelizacion/recompensas", data={"name": "x", "points_cost": "1"}
        )
