...
def business_reservar_wizard_confirmar(slug):
    ...
    if request.method == "POST":
        ...
        if not nombre or len(nombre) < 2:
            return jsonify({"success": False, "error": "El nombre y apellido son obligatorios (mínimo 2 caracteres)."}), 400
        if not telefono:
        ...
        # Validar servicio
        allowed_services = {row["name"] for row in get_active_services_scoped(business_id)} if business_id else set()
        if servicio not in allowed_services:
            return jsonify({"success": False, "error": "El servicio seleccionado no es válido."}), 400
        ...
        if not resultado.get("success"):
            ...
            code_map = {
                ...
                "invalid_service": ("invalid_service", "El servicio seleccionado no está disponible."),
                "invalid_resource": ("invalid_resource", "El recurso seleccionado no es válido o no está disponible."),
                "invalid_name": ("invalid_name", "El nombre y apellido deben tener al menos 2 caracteres."),
