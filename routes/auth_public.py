...
from extensions import _is_request_allowed, valid_csrf_token
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        if not _is_request_allowed(
            _rate_limit_key("forgot", get_client_ip()), FORGOT_REQUEST_LIMIT
        ):
    ...
    return render_template(
        "forgot.html", business_name=business_name, sent=request.args.get("sent") == "1"
    )
...
def registro():
    ...
    if request.method == "POST":
        ...
        if not _is_request_allowed(
            ...
            return redirect(
                url_for(
                    "registro", error=" Demasiadas solicitudes. Intentá nuevamente en unos minutos."
                )
            )
    ...
    business_name = (
        settings["business_name"] if settings and settings["business_name"] else "TurnoBot"
    )
    return render_template("registro.html", business_name=business_name, sent=sent, error=error)
...
def reset_password(token):
    ...
    if request.method == "GET":
        return render_template("reset.html", invalid=False, email=reset_token["user_email"])
